...
- **P0-2** [African-Student Reality Check]: `install-sentinel.ps1` and `install-overmind.ps1` run `pip install git+https://github.com/...` which can pull 200 MB of dependencies. On a 2 Mbps connection with a 500 MB monthly data cap (typical Ugandan student), **one install consumes a week's data budget**. There is no `--offline` fixture, no pre-bundled wheel, no size-estimate preflight. Suggested fix: (a) add `--estimate-mb` dry-run that downloads metadata only; (b) offer an "essentials-only" mode that skips pip-heavy chains; (c) package Sentinel+Overmind as a combined wheel hosted on the binary-mirror repo for one-shot download.
