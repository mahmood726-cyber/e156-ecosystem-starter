...
| **Dashboard kit** | **v0.6.0** | `scripts/install-rapidmeta.ps1` clones [`rapidmeta-kit`](https://github.com/mahmood726-cyber/rapidmeta-kit) at a pinned commit and persists `RAPIDMETA_KIT_PATH`. `clone.py` turns a small JSON config into a self-contained meta-analysis **dashboard** — **stdlib-only, offline, zero deps, no LLM**. Run `bash run_example.sh` for a token-free example. *(The upstream repository, its clone URL, its installer filenames and its environment variable keep their published names — those are identifiers of a real remote, and renaming them here would break the installer. The layer is referred to by function everywhere else.)* |
...
python scripts/meta-self-audit.py MY_REVIEW.html --json     # for CI or an agent

# optional: also check your pooling code against invariants any correct
# implementation must satisfy (reuses Overmind's MetamorphicWitness)
...
cannot establish the facts it needs returns `INCONCLUSIVE` and exits non-zero,
because "I could not check this" silently becoming "this is fine" is itself one
of the catalogued error classes. A crashed detector is inconclusive, never clean.

**Read the blind spots before you trust a `CLEAN`.** They are listed next to the
detectors in the library — the checks are text-pattern matching over one
rendered page, they cannot see your extraction sheet or your actual search, and
nothing here recomputes your pooled estimate from raw data.

Wire it into a pre-push hook alongside Sentinel if you want it enforced:
...
```bash
python templates/count-recovery/count_recovery_harness.py --selftest   # 14 checks, 11 negative controls
python templates/count-recovery/count_recovery_harness.py --chain      # retrieval fallback order
python templates/count-recovery/count_recovery_harness.py example_extraction.json
```
