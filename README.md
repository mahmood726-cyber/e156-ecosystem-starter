...
> **The four layers above let students recreate Mahmood's meta-analysis tooling end-to-end.** Each defaults to an **offline / low-token** path and keeps heavy deps opt-in — see [`STUDENT-TOKEN-BUDGET.md`](STUDENT-TOKEN-BUDGET.md) for where tokens are (and aren't) spent.
